...
import {
	buildTodoWriteResultText,
	executeTodoWriteOperation,
} from "./todo-write-shared.js";
...
import {
	loadClaudeSubagentActiveTeamName,
	loadClaudeSubagentActiveTeamNameSync,
} from "./claude-subagent-integration.js";
import {
	buildTeammateRuntimeTools,
	clearClaudeTodoBridge,
	listLiveTeammates,
	listManagedTasks,
	registerClaudeTodoBridge,
} from "./subagent-runtime-integration.js";
import {
	buildTaskSummary,
	createTask,
	deleteTask,
...
function buildTaskResultText(
	details: TaskCreateDetails | TaskUpdateDetails,
): string {
	if (!details.success) {
...
function renderTaskAssignmentMessage(
	message: any,
	...
	if (!details) {
		return new Text(
			typeof message.content === "string" ? message.content : "Task assigned",
			0,
			0,
		);
	}
	...
	let text = theme.fg(
		"accent",
		theme.bold(`Task #${details.taskId} assigned to @${details.owner}`),
	);
	...
	return events
		.filter((event) => {
			...
			return Number.isNaN(timestamp)
				? true
				: now - timestamp <= RECENT_EVENT_TTL_MS;
		})
...
export default function claudeTodoV2(pi: ExtensionAPI): void {
	...
	const rememberRecentEvent = (event: RecentCollabEvent): void => {
		...
		const currentTaskListId = normalizeTaskListId(
			lastUiContext
				? getResolvedTaskListId(lastUiContext)
				: getResolvedTaskListId(),
		);
	...
	pi.events.on("claude-subagent:teammates-changed", (event) => {
		...
		if (teamName && typeof payload?.name === "string") {
			if (payload.status === "completed") {
				const summary =
					typeof payload.lastResultText === "string" &&
	...
	function applyWorkerToolSelection(): void {
		...
		const activeTools = [
			...new Set([...configuredTools, ...TOOL_NAMES]),
	...
	async function refreshLinkedTaskListContext(cwd: string): Promise<void> {
		cachedSubagentTeamTaskListId = normalizeTaskListId(
			await loadClaudeSubagentActiveTeamName(cwd),
		);
	}
	...
	async function restoreState(ctx: ExtensionContext): Promise<void> {
		...
		for (const entry of ctx.sessionManager.getBranch()) {
			...
			if (
				...
				isPersistedState(customEntry.data)
			) {
	...
	function shouldShowTaskPanelStatus(): boolean {
		return (pi.getFlag("task_panel") as boolean | undefined) === true;
	}

	function getResolvedTaskListId(ctx?: {
		sessionManager: { getSessionId(): string };
	}): string {
		const flagValue = normalizeTaskListId(
			pi.getFlag("claude-todo-v2-task-list") as string | undefined,
		);
		if (flagValue) return flagValue;

		const envValue = normalizeTaskListId(
			process.env.PI_CLAUDE_TODO_V2_TASK_LIST_ID ??
		...
		const configTaskListId = normalizeTaskListId(
			(pi.getFlag("claude-todo-v2-config-task-list") as string | undefined) ??
				cachedConfigTaskListId,
		);
		if (configTaskListId) return configTaskListId;

		const liveTeamTaskListId = normalizeTaskListId(
			loadClaudeSubagentActiveTeamNameSync(process.cwd()),
		);
	...
	async function setTaskListOverride(
	...
	function getManagedTaskStatusColor(
		status: ManagedTaskSnapshot["status"],
	): keyof Theme {
		switch (status) {
			case "running":
				return "accent";
	...
	function buildManagedTaskLines(
		theme: Theme,
		managedTasks: ManagedTaskSnapshot[],
	): string[] {
		...
		for (const task of managedTasks) {
			...
			const detail = previewManagedText(
				task.error ?? task.resultText ?? task.description,
			);
			const suffix = detail ? ` - ${detail}` : "";
			lines.push(
				theme.fg(
					color,
					`${task.runtimeName} [${task.taskId}]: ${task.status} (${task.agentType})${suffix}`,
				),
			);
		}
		return lines;
	...
	async function maybeAutoClaimIdleTeammates(
		taskListId: string,
	): Promise<void> {
		...
		const teammates = listLiveTeammates(taskListId)
			...
			.filter(
				(teammate) =>
					teammate.status === "idle" || teammate.status === "completed",
			);
	...
	async function refreshUi(
		...
		const config = await getConfig();
		const tasks = filterExternalTasks(
			await listTasks(process.cwd(), taskListId),
		);
		const teammates = listLiveTeammates(taskListId);
		const managedTasks = listManagedTasks(taskListId);
		recentEvents = trimRecentEvents(recentEvents);
		const relevantRecentEvents = recentEvents.filter(
			(event) => event.taskListId === taskListId,
		);
		...
		if (
			...
			(tasks.length === 0 &&
				...
				relevantRecentEvents.length === 0)
		) {
			clearUi(ctx);
			return;
		}

		if (shouldShowTaskPanelStatus()) {
			const baseStatus = buildStatusText(
				ctx.ui.theme,
				taskListId,
				tasks,
				workers,
			...
			const runningManaged = managedTasks.filter(
	...
	async function formatCurrentTaskList(taskListId: string): Promise<string> {
		...
		return formatTaskListText(
			taskListId,
			tasks.map((task) => buildTaskSummary(task, resolvedIds)),
		);
	}
	...
	function shouldSendReminder(
		...
		const sinceWrite =
			config.reminders?.turnsSinceWrite ?? TURNS_SINCE_WRITE_DEFAULT;
		const between =
			config.reminders?.turnsBetweenReminders ??
			TURNS_BETWEEN_REMINDERS_DEFAULT;
		...
		pi.registerTool({
			...
			async execute(
				...
				const details: TaskCreateDetails = {
					...
					...(hookResult.blocked
						? { error: `TaskCreated hook feedback:\n${hookResult.message}` }
						: {}),
				};
			...
			renderResult(result, _options, theme) {
				...
				const text = details
					...
					: result.content[0]?.type === "text"
						...
						: "";
				return new Text(
					details?.success === false ? theme.fg("error", text) : text,
					0,
					0,
				);
			},
		...
		pi.registerTool({
			...
			async execute(
				...
				const summaries = tasks.map((task) =>
					buildTaskSummary(task, resolvedIds),
				);
				...
				return {
					content: [
						{ type: "text", text: formatTaskListText(taskListId, summaries) },
					],
					details,
				};
			},
		...
		pi.registerTool({
			...
			async execute(
				...
				const details = await executeTaskUpdateOperation({
					...
					buildCustomTools: (actingAgentName) =>
						buildTeammateRuntimeTools({
							cwd: process.cwd(),
							taskListId,
							actingAgentName,
	...
	function registerCommands(): void {
		pi.registerCommand(COMMAND_NAME, {
			...
			handler: async (args: string, ctx: ExtensionCommandContext) => {
				lastUiContext = ctx;
				const [first = "show", second, third] = args
					.trim()
					.split(/\s+/)
					.filter(Boolean);
				const taskListId = getResolvedTaskListId(ctx);
				...
				if (first === "use") {
					...
					ctx.ui.notify(
						second
							? `Task list override set to ${normalizeTaskListId(second)}`
							: "Task list override cleared.",
						"info",
					);
					await refreshUi(ctx);
					return;
				...
				if (first === "panel") {
					...
					await refreshUi(ctx);
					ctx.ui.notify(
						`Task panel ${state.panelEnabled ? "enabled" : "disabled"}.`,
						"info",
					);
					return;
				}
				...
				if (first === "workers") {
					if (second === "start") {
						...
						await workerManager.start(
							Number.isNaN(parsedCount) ? 1 : parsedCount,
						);
						await refreshUi(ctx);
					...
					const text =
						workerManager.list().length === 0
							? "No workers running."
							: workerManager
									.list()
									.map(
										(worker) =>
											`${worker.name}: ${worker.status}${worker.currentTaskId ? ` #${worker.currentTaskId}` : ""}${worker.message ? ` - ${worker.message}` : ""}`,
									)
									.join("\n");
				...
				await ctx.ui.editor(
					`Claude Todo V2 task list: ${taskListId}`,
					await formatCurrentTaskList(taskListId),
				);
			},
		});
	}

	pi.registerFlag("task_panel", {
		description: "Show Claude Todo V2 task status in the global status bar",
		type: "boolean",
	});
	...
	pi.registerFlag("claude-todo-v2-config-task-list", {
		description:
			"Internal override used by wrappers/tests to seed a default task list ID",
		type: "string",
	});
	...
	pi.on("context", async (event: any, ctx: ExtensionContext) => {
		const filteredMessages = event.messages.filter(
			(message: { customType?: string }) =>
				message.customType !== TASK_CONTEXT_CUSTOM_TYPE,
		);
	...
	pi.on(
		...
		async (_event: unknown, ctx: ExtensionContext) => {
			...
			const taskCount = filterExternalTasks(
				await listTasks(process.cwd(), taskListId),
			).length;
			return {
				message: {
